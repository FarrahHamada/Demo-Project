...
// message board routes ===============================================================
    ...
    var insertDocuments2 = function(db, req, filePath, callback) {
      console.log(req.session.passport.user,"user1",req.user)
      req.user.local.profileImg = filePath
      console.log(req.user, "copy")
        var collection = db.collection('users');
        var uId = ObjectId(req.session.passport.user)
        collection.findOneAndUpdate({"_id": uId}, {
          $set: {
    ...
    app.post('/profileImg', upload.single('file-to-upload'), (req, res, next) => {

    insertDocuments2(db, req, 'img/' + req.file.filename, () => {
        //db.close();
        //res.json({'message': 'File uploaded successfully'});
        res.redirect('/profile')
    });
